...
class WPBDP_RegionFieldsAPI {
    ...
    public function field_attributes(&$field, $selected, $display_context) {
        ...
        $should_hide = $should_hide || !$field->has_display_flag( 'region-selector' );
        // do not render field options if the field's level is below the
        // min visible level
        $should_hide = $should_hide || $level < $min;

        if ( $should_hide ) {
            $field->css_classes[] = 'wpbdp-regions-hidden';
        } else if ( $level >= $min ) {
            $field->set_data('options', $this->_field_options($field, $level, $selected, $parent));
        }
    }
    ...
    private function _field_options($field, $level, $selected, $parent) {
        $api = wpbdp_regions_api();
