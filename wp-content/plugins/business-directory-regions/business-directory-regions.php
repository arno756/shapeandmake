...
class WPBDP_RegionsPlugin {
    ...
    public function register_settings($settings) {
        ...
        $settings->add_setting($s, 'regions-hide-selector', _x('Hide Region selector?', 'admin settings', 'wpbdp-regions'), 'boolean', false);
    ...
    public function pre_get_posts(&$query) {
        ...
        if (!$query->is_admin && $query->get('post_type') == WPBDP_POST_TYPE && !$query->get('post__in')) {
            $api = wpbdp_regions_api();
            $id = $api->get_active_region();

            if (is_null($id)) return;

            $region = $api->find_by_id($id);

            if (is_null($region)) return;

            $tax_query = array_filter((array) $query->get('tax_query'));
            $tax_query[] = array('taxonomy' => $taxonomy, 'field' => 'id', 'terms' => $id);
    ...
    // /**
    //  * Taken from WPBDP api/listings.php
    //  *
    //  * @param  string $link
    //  * @param  object $term
    ...
    //     if ( ($taxonomy == self::TAXONOMY) && (_wpbdp_template_mode('category') == 'page') ) {
    //         if (wpbdp_rewrite_on()) {
    //             return rtrim(wpbdp_get_page_link('main'), '/') . '/' . self::TAXONOMY . '/' . $term->slug . '/';
    //         } else {
    //             return add_query_arg('region', $term->slug, wpbdp_get_page_link('main')); // XXX
    //         }
    //     }
