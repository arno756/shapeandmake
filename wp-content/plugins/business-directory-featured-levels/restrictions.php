...
class WPBDP_FeaturedLevelsModule_Restrictions {
    ...
    public function _fee_description( $description, $fee ) {
        ...
        // character limit
        $char_limit = $this->get_char_limit( array( 'fee' => $fee ) );
    ...
    public function get_char_limit_from_state( &$state ) {
        $listing_level = $this->get_listing_level( $state );
        $limit = abs( $this->get_char_limit( array( 'level' => $listing_level ) ) );        

        foreach ( $state->categories as $fee_id ) {
            $flimit = $this->get_char_limit( array( 'fee' => $fee_id ) );
    ...
    public function _validate_length( $validates, &$errors, &$field, $value, &$state ) {
        if ( $field->get_association() != 'content' || !$validates )
            return $validates;

        $limit = $this->get_char_limit_from_state( $state );
    ...
    public function _character_limit_data_attrs( $htmlattrs, &$field, $value, $render_context, &$extra ) {
        if ( $render_context != 'submit' || $field->get_association() != 'content' )
            return $htmlattrs;

        $htmlattrs = is_array( $htmlattrs ) ? $htmlattrs : array();

        if ( $limit = $this->get_char_limit_from_state( $extra ) ) {
    ...
    public function _character_limit_display( $field_inner, &$field, $value, $render_context, &$extra ) {
        ...
        $limit = $this->get_char_limit_from_state( $extra );
        if ( $limit == 0 )
            return $field_inner;
    ...
    public function get_char_limit( $args ) {
        $args = wp_parse_args( $args, array( 'listing' => null, 'fee' => null, 'level' => null ) );
        ...
        if ( !is_null( $args['listing'] ) && !$listing_id )
            $limit = max( abs( $this->get_char_limit( array( 'fee' => 0 ) ) ), abs( $this->get_char_limit( array( 'level' => 'normal' ) ) ) );

        if ( $listing_id > 0 ) {
            $listing_fees = wpbdp_listings_api()->get_listing_fees( $listing_id );
            $listing_level = wpbdp_listing_upgrades_api()->get_listing_level( $listing_id )->id;

            $llimit = $this->get_char_limit( array( 'level' => $listing_level ) );
            ...
            foreach ( $listing_fees as &$lfee ) {
                ...
                $flimit = $this->get_char_limit( array( 'fee' => $fee['id'] ) );
            ...
            $limit = isset( $restrictions['fees'][ $fee_id ] ) ? wpbdp_getv( $restrictions['fees'][ $fee_id ], 'char_limit', 0 ) : 0;
        } elseif ( $level_id ) {
            $limit = isset( $restrictions['levels'][ $level_id ] ) ? wpbdp_getv( $restrictions['levels'][ $level_id ], 'char_limit', 0 ) : 0;
        }
        ...
        switch ( $group ) {
            ...
            case 'misc':
                ...
                foreach ( array( 'levels', 'fees' ) as $o ) {
                    ...
                    foreach ( $_POST[ $o ] as $item => $char_limit ) {
                        $config[ $o ][ $item ]['char_limit'] = abs( intval( $char_limit ) );
                    }
                }

                $this->save_restrictions( $config, 'char_limit' );
    ...
    private function settings_form_misc() {
        ...
        foreach ( $featured_levels as &$level ) {
            $level_config[] = (object) array( 'level' => &$level,
                                              'char_limit' => $this->get_char_limit( array( 'level' => $level->id ) )
                                            );
        }
        ...
        foreach ( $fee_plans as &$fee ) {
            $fees_config[] = (object) array( 'fee' => $fee,
                                             'char_limit' => $this->get_char_limit( array( 'fee' => $fee->id ) )
                                  );
        }
